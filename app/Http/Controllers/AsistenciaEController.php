...
class AsistenciaEController extends Controller
{
    public function index()
    {
        ...
        $user = auth()->user();
        // Traer solo las asistencias del usuario logueado
        $asistencias = Asistencia::with('empleadoSucursal')
            ->where('estado', 1) // Solo asistencias activas
            ->where('id_empleado_sucursal', $user->id) // Filtra por el ID del usuario logueado
            ->get();
    
        // Verificar si hay asistencias y devolver la vista
        return view('Asistencias.IndexE', compact('asistencias'));
    }
